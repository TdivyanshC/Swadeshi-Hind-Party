...
function ShareModal({ isOpen, onClose, objective }: { isOpen: boolean; onClose: () => void; objective: any }) {
  const [copied, setCopied] = useState(false);

  const shareUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/objectives#objective-${objective?.id}`;
...
export const objectivesData = [
  ...
  },
  {
    ...
    description: "Guaranteeing employment for at least one qualified member in every household based on their educational qualifications",
    icon: (
      <svg className="w-16 h-16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
